...
namespace Femyou.internals {
	internal class Library : IDisposable {
		...
		public Library(string baseFolder, string name) : this(Platform.GetLibraryPath(Environment.OSVersion.Platform,
			Environment.Is64BitOperatingSystem, baseFolder, name)) { }
		...
		public Library(string path) {
			...
			fmi2EnterInitializationMode =
				fmuLibrary.LoadFunction<FMI2.fmi2EnterInitializationModeTYPE>(nameof(fmi2EnterInitializationMode));
			fmi2ExitInitializationMode =
				fmuLibrary.LoadFunction<FMI2.fmi2ExitInitializationModeTYPE>(nameof(fmi2ExitInitializationMode));
