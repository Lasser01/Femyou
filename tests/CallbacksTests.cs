...
namespace Femyou.Tests {
	public class CallbacksTests {
		...
		public void LogWhenVariableCannotBeSet() {
			...
			using IModel model = Model.Load(TestTools.GetFmuPath("BouncingBall.fmu"));
			using IInstance instance = model.CreateCoSimulationInstance("my name", spyCallback);
			try {
		...
		private class SpyCallback : ICallbacks {
